...
class Session(OrgModelMixin):
    ...
    SUFFIX_MAP = {1: '.gz', 2: '.replay.gz', 3: '.cast.gz', 4: '.replay.mp4', 5: '.replay.json'}
    ...
    def get_replay_local_storage_path(self, rel_path):
        return os.path.join(self.upload_to, rel_path)
    ...
    def save_replay_to_storage_with_version(self, f, version=2):
        if version <= 4:
            ...
            local_path = self.get_replay_local_storage_path(rel_path)
        try:
