...
from authentication.backends.ldap import LDAPAuthorizationBackend, LDAPUser, LDAPHAAuthorizationBackend
...
class LDAPConfig(object):
    ...
    def load_from_settings(self):
        if self.category == 'ldap':
            ...
            self.server_uri = settings.AUTH_LDAP_HA_SERVER_URI
            self.bind_dn = settings.AUTH_LDAP_HA_BIND_DN
            self.password = settings.AUTH_LDAP_HA_BIND_PASSWORD
            self.use_ssl = settings.AUTH_LDAP_HA_START_TLS
            self.search_ou = settings.AUTH_LDAP_HA_SEARCH_OU
            self.search_filter = settings.AUTH_LDAP_HA_SEARCH_FILTER
            self.attr_map = settings.AUTH_LDAP_HA_USER_ATTR_MAP
            self.auth_ldap = settings.AUTH_LDAP_HA
